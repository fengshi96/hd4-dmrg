...
###################
	...
	plt.plot(np.abs(CrONE[:,1]/CrONE[2,1]),marker="s",linewidth=2,color='red',markersize=markersize,markeredgewidth=0,label=r"$C_{L}(R)$")
	#plt.plot(np.abs(CrONE[:,2]/CrONE[2,2]),'o-',color='blue',label=r'$C_{LS}(R)$')
	plt.plot(np.abs(CrONE[:,3]/CrONE[3,1]),marker="^",linewidth=2,color='teal',markersize=markersize,markeredgewidth=0,label=r"$C_{J}(R)$")
	plt.yscale('log')
	plt.xscale("log")
	plt.ylim(ymax=10)
