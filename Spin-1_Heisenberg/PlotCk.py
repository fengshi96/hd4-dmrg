...
###################
	...
	#titlelabel = " Lambda = "+Lambda
	#plt.title(titlelabel,fontsize=16, color='black')
	plt.plot(Sk/(Ns*Ns),'o-',color='black',markersize=markersize,markeredgewidth=0,label=r'$S(K)$')
	plt.plot(Lk/(Ns*Ns),'s-',color='blue',markersize=markersize,markeredgewidth=0,label=r'$L(K)$')
	#plt.plot(SLk/(Ns*Ns),'o-',color='blue',label=r'$SL(K)$')
	plt.plot(Jk/(Ns*Ns),'^-',color='red',markersize=markersize,markeredgewidth=0,label=r'$J(K)$')
	plt.grid(which='major',linestyle='--',alpha=0.35)
	...
	ax.annotate("(a)", xy=(0.05,0.92), xycoords="axes fraction",fontsize=20)
	plt.savefig('NNSS_CK.pdf')
	plt.close()
